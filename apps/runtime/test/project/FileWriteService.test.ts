...
describe('FileService.write', () => {
  ...
  it('commits a batch of writes together through the project boundary', async () => {
    const root = await mkdtemp(join(tmpdir(), 'idle-file-write-batch-'));
    await writeFile(join(root, 'a.txt'), 'a');
    await writeFile(join(root, 'b.txt'), 'b');
    const projects = new ProjectService();
    const project = await projects.open(root);
    const files = new FileService(projects);
    await files.writeBatch(project.id, [
      { path: 'a.txt', content: 'A' },
      { path: 'b.txt', content: 'B' },
    ]);
    await expect(readFile(join(root, 'a.txt'), 'utf8')).resolves.toBe('A');
    await expect(readFile(join(root, 'b.txt'), 'utf8')).resolves.toBe('B');
  });
});
