...
export class ChangeSetVerificationError extends Error {
  ...
  constructor(errors: ContractChangeSetVerificationError[], rolledBack = false, rollbackError: string | null = null) {
    super(`Change Set verification failed: ${errors.map((error) => error.path).join(', ')}`);
    this.name = 'ChangeSetVerificationError'; this.errors = errors; this.rolledBack = rolledBack; this.rollbackError = rollbackError;
  }
}
...
export class ChangeSetService {
  constructor(private readonly projects: ProjectService, private readonly files: FileService) {}
  ...
  private buildPreview(changeSet: ChangeSet, states: Map<string, ChangeFileState>, result: ReturnType<typeof applyChangeSet>): ChangeSetPreviewResult {
    ...
    return { id: changeSet.id, files: changeSet.changes.map((change) => {
      const current = states.get(change.path)?.content ?? null;
      const next = changesByPath.get(change.path)?.content ?? null;
      return { path: change.path, operation: change.operation, oldContent: change.operation === 'create' ? null : current, newContent: next, additions: Math.max(0, lineCount(next) - lineCount(current)), deletions: Math.max(0, lineCount(current) - lineCount(next)) };
    }) };
  }
  ...
  async review(projectId: string, changeSet: ChangeSet): Promise<ChangeSetReviewResult> {
    ...
    if (changeSet.changes.length === 0) return { id: changeSet.id, valid: true, errors: [], preview: { id: changeSet.id, files: [] } };
    ...
    try {
      ...
      const errors = (error as { errors?: ChangeSetValidationError[] }).errors ?? [{ path: '', code: 'BASE_MISMATCH' as const, message: error instanceof Error ? error.message : String(error) }];
  ...
  async apply(projectId: string, changeSet: ChangeSet): Promise<ChangeSetApplyResult> {
    ...
    const states = await this.readStates(projectId, changeSet);
    const result = applyChangeSet(changeSet, states);
    await this.files.applyBatch(projectId, result.changes.map((change) => ({ path: change.path, content: change.content })));
    ...
    for (const change of result.changes) {
      ...
      if (!matches) verificationErrors.push({ path: change.path, code: 'VERIFY_MISMATCH', message: expectedExists ? 'The applied file content does not match the planned result.' : 'The applied file still exists after a planned delete.' });
      else verifiedFiles.push(change.path);
    }
    if (verificationErrors.length > 0) {
      try {
        await this.files.restoreBatch(projectId, result.changes.map((change) => ({ path: change.path, state: states.get(change.path) ?? { exists: false, content: '' } })));
        ...
        throw new ChangeSetVerificationError(verificationErrors, false, error instanceof Error ? error.message : String(error));
      }
    }
    return { id: changeSet.id, changedFiles: result.changes.map((change) => change.path), verifiedFiles };
  }
}
