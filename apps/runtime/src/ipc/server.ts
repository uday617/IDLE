...
export interface RuntimeServer {
  start(): Promise<void>; stop(): Promise<void>; health(): RuntimeHealth;
...
export function createRuntimeServer(version: string): RuntimeServer {
  ...
  taskRunner.subscribe((event) => {
    ...
    const contractEvent: TaskStatusEvent = { taskId: event.taskId as TaskStatusEvent['taskId'], status, timestamp: event.timestamp, ...(event.error ? { message: event.error } : {}) };
  ...
  return {
    ...
    async submitTask(request) {
      ...
      void taskRunner.submit({ id: request.taskId, projectId: request.projectId, prompt: request.prompt, checkpoint: { name: 'submitted', data: { projectId: request.projectId, prompt: request.prompt } } });
    ...
    async getTask(taskId) {
      ...
      if (task.checkpoint?.name === 'agent.changeset') {
        ...
        if (typeof checkpoint?.id === 'string') {
          ...
          if (changeSet) result.changeSetReview = await changeSetService.review(task.projectId, changeSet);
        }
      }
      return result;
    },
