...
export function createRuntimeServer(version: string): RuntimeServer {
  ...
  const agentExecutor = new AgentExecutor(projectService, fileService); const agentPlanner = new AgentPlanner(); const changeSetBuilder = new AgentChangeSetBuilder();
  const taskRunner = new TaskRunner(taskService, async (request) => {
    ...
    const changeSet = changeSetBuilder.createChangeSet(plan); generatedChangeSets.set(changeSet.id, changeSet); await taskService.checkpoint(request.id, { name: 'agent.changeset', data: changeSet });
  });
