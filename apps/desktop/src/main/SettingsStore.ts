...
export class SettingsStore {
  ...
  async get(): Promise<ProviderSettings> {
    ...
    try {
      ...
      return { baseUrl: stored.baseUrl || defaults.baseUrl, model: stored.model || defaults.model, timeoutMs: stored.timeoutMs || defaults.timeoutMs, ...(apiKey ? { apiKey } : {}) };
    } catch {
      return defaults;
    }
  }
  ...
  async set(settings: ProviderSettings): Promise<ProviderSettings> {
    if (!settings.baseUrl.trim() || !settings.model.trim()) throw new Error('Model and base URL are required');
