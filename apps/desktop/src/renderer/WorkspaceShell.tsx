...
export function WorkspaceShell() {
  ...
  useEffect(() => window.idle.tasks.subscribe((event: TaskStatusEvent) => { if (taskIdRef.current !== event.taskId) return; setTaskStatus(event.status); setTaskError(event.message ?? null); }), []);
  useEffect(() => {
    if (!taskId) return;
    let cancelled = false;
    const poll = async () => {
  ...
  const applyChanges = async () => {
    ...
    setApplying(true);
    try { await window.idle.project.applyChangeSet(state.project.id, changeSet); setTaskError(null); setReview(null); setChangeSet(null); } catch (cause) { setTaskError(cause instanceof Error ? cause.message : 'Unable to apply changes'); } finally { setApplying(false); }
  };
  ...
  return <main className="workspace-shell">
    ...
    <section className="workspace-grid">
      ...
      <section className="panel editor-panel"><div className="panel-header editor-header"><div><h2>{getWorkspacePanelTitle('editor')}</h2><span className="panel-subtitle">{selectedFile ?? 'No file selected'}</span></div><span className="readonly-badge">LOCAL</span></div><Editor projectId={state.project?.id ?? null} filePath={selectedFile}/>{taskResult ? <TaskWorkspace result={taskResult} prompt={taskPrompt} orchestration={orchestration}/>: null}<DeveloperToolsPanel projectId={state.project?.id ?? null}/></section>
