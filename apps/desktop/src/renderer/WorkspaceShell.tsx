...
import type { ChangeSetReviewResult, ProjectId, TaskId, TaskResult, TaskStatus, TaskStatusEvent } from '@idle/contracts';
...
import { getAgentPanelState, getWorkspaceStatus, workspaceActions } from './workspaceUiModel.js';
...
export function WorkspaceShell() {
  ...
  const submitTask = async () => { const prompt = taskPrompt.trim(); if (!state.project || !prompt || inputMode !== 'task') return; const nextTaskId = crypto.randomUUID() as TaskId; taskIdRef.current = nextTaskId; setTaskId(nextTaskId); setTaskStatus('queued'); setTaskError(null); setReview(null); setChangeSet(null); try { await window.idle.tasks.submit({ taskId: nextTaskId, projectId: state.project.id as ProjectId, prompt }); setTaskPrompt(''); } catch (cause) { setTaskStatus('failed'); setTaskError(cause instanceof Error ? cause.message : 'Unable to submit task'); } };
  const applyChanges = async () => { if (!state.project || !changeSet || !review?.valid) return; setApplying(true); try { await window.idle.project.applyChangeSet(state.project.id, changeSet); setTaskError(null); setReview(null); setChangeSet(null); } catch (cause) { setTaskError(cause instanceof Error ? cause.message : 'Unable to apply changes'); } finally { setApplying(false); } };
  const workspaceStatus = getWorkspaceStatus({ projectPath: state.project?.path ?? null, dirty: false, agentCount: taskStatus ? 1 : 0 });

  return <main className="workspace-shell">
    <header className="titlebar"><div className="brand-block"><strong>IDLE</strong><span>Multi-agent coding workspace</span></div><div className="top-actions"><button className="action-button" type="button" onClick={() => void openProject()} disabled={state.opening}>{state.opening ? 'Opening…' : 'Open Project'}</button><button className="action-button primary-action" type="button" onClick={() => focusInput('task')} disabled={!state.project}>+ Quick Task</button><button className="icon-button" type="button" onClick={() => focusInput('command')} aria-label="Command search">⌕</button></div></header>
    <div className="taskbar"><label className="quick-task-input"><span>{inputMode === 'command' ? '⌕' : '✦'}</span><input ref={taskInputRef} value={taskPrompt} onChange={(event) => setTaskPrompt(event.target.value)} onKeyDown={(event) => { if (event.key === 'Enter') void submitTask(); }} placeholder={!state.project ? 'Open a project to start a task' : inputMode === 'command' ? 'Search commands, files, and actions…' : 'Ask IDLE to change, explain, or verify something…'} disabled={!state.project}/><kbd>Enter</kbd></label><span className="taskbar-hint">{taskStatus && taskId ? `Task ${taskId.slice(0, 8)} · ${taskStatusLabel[taskStatus]}${taskError ? ` · ${taskError}` : ''}` : 'AI changes will be reviewed before they are applied.'}</span></div>
    <section className="workspace-grid">
      ...
      <section className="panel editor-panel"><div className="panel-header editor-header"><div><h2>{getWorkspacePanelTitle('editor')}</h2><span className="panel-subtitle">{selectedFile ?? 'No file selected'}</span></div><span className="readonly-badge">LOCAL</span></div><Editor projectId={state.project?.id ?? null} filePath={selectedFile}/></section>
      <aside className="panel agents-panel"><div className="panel-header"><div><h2>{emptyAgentPanel.heading}</h2><span className="panel-subtitle">{taskStatus ? taskStatusLabel[taskStatus] : emptyAgentPanel.summary}</span></div><span className="agent-count">{taskStatus ? '1' : '0'}</span></div><div className="agent-empty-state"><div className="agent-orbit">{taskStatus ? '●' : '✦'}</div><strong>{taskStatus ? taskStatusLabel[taskStatus] : 'Ready for agent work'}</strong><p>{taskError ?? (taskStatus ? 'Task lifecycle is connected to the runtime.' : 'Start a Quick Task and IDLE will show planning, execution, verification, and review here.')}</p><button className="secondary-button" type="button" onClick={() => focusInput('task')} disabled={!state.project}>{taskStatus ? 'Start another task' : 'Start a task'}</button></div>{review ? <ChangeSetReview review={review} applying={applying} onApply={() => void applyChanges()}/>: null}</aside>
    </section>
